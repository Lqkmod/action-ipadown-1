...
async def stop_all_treo_tasks_for_user(user_id_str: str, context: ContextTypes.DEFAULT_TYPE, reason: str = "Unknown") -> int:
    ...
    # Lấy những task đang chạy nhưng không có trong config (có thể do lỗi trước đó)
    targets_in_runtime_only = list(active_treo_tasks.get(user_id_str, {}).keys() - set(targets_in_persistent))

    targets_to_process = set(targets_in_persistent) | set(targets_in_runtime_only) # Kết hợp cả hai để đảm bảo dừng hết
    ...
    for target_username in targets_to_process:
        # Hàm stop_treo_task sẽ xử lý cả runtime và persistent
        if await stop_treo_task(user_id_str, target_username, context, reason):
            stopped_count += 1
        else:
             # Log cảnh báo nếu không dừng được task/config lẽ ra phải tồn tại
...
async def cleanup_expired_data(context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, data in list(valid_keys.items()):
        ...
        except (ValueError, TypeError): keys_to_remove.append(key) # Xóa nếu dữ liệu lỗi
    ...
    for user_id_str, expiry_timestamp in list(activated_users.items()):
        ...
        except (ValueError, TypeError): users_to_deactivate_key.append(user_id_str) # Xóa nếu dữ liệu lỗi
    ...
    for user_id_str, vip_data in list(vip_users.items()):
        try:
            ...
            users_to_deactivate_vip.append(user_id_str) # Xóa nếu dữ liệu lỗi
    ...
    for user_id_str, targets_data in user_daily_gains.items():
        ...
        for target_username, gain_list in targets_data.items():
            # Lọc giữ lại các entry trong ngưỡng thời gian
            ...
            if len(valid_gains) < len(gain_list): # Nếu có entry bị xóa
                ...
                else: targets_to_remove_from_user.append(target_username) # Xóa target nếu không còn gain
            elif not valid_gains and not gain_list: targets_to_remove_from_user.append(target_username) # Xóa target nếu list rỗng

        # Xóa các target không còn gain của user
    ...
    # Stop tasks for expired/invalid VIPs <-- Logic mới quan trọng
...
async def call_follow_api(user_id_str: str, target_username: str, bot_token: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient(verify=False, timeout=90.0) as client: # Tăng timeout
            resp = await client.get(FOLLOW_API_URL_BASE, params=api_params, headers={'User-Agent': 'TG Bot FL Caller'})
            content_type = resp.headers.get("content-type", "").lower()
            response_text_full = ""
            try: # Thử decode với nhiều encoding
            ...
            response_text_for_debug = response_text_full[:1000] # Giới hạn log
...
async def menu_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer() # Luôn trả lời callback
    ...
    # Lấy tên lệnh từ callback_data (vd: show_muatt -> /muatt)
    command_name = callback_data.split('_')[-1]
    fake_message = Message(message_id=query.message.message_id + 1, # ID giả, không quan trọng
                           date=datetime.now(), chat=query.message.chat, from_user=user, text=f"/{command_name}")
...
# --- Lệnh /lenh (Đã sửa lỗi SyntaxError) ---
async def lenh_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_vip:
        ...
        expiry_str = "Không rõ"
        if expiry_ts:
            try: # <<< Sửa lỗi SyntaxError ở đây
                expiry_str = datetime.fromtimestamp(float(expiry_ts)).strftime('%d/%m/%Y %H:%M')
...
# --- Lệnh /nhapkey (Đã sửa lỗi SyntaxError) ---
async def nhapkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not key_data:
        ...
        if activation_time_ts:
            try: # <<< Sửa lỗi SyntaxError ở đây
                used_time_str = f" lúc {datetime.fromtimestamp(float(activation_time_ts)).strftime('%H:%M:%S %d/%m/%Y')}"
            except (ValueError, TypeError, OSError):
                used_time_str = " (lỗi thời gian)" # Gán giá trị nếu lỗi
        ...
        if expiry_time_ts:
            try: # <<< Sửa lỗi SyntaxError ở đây
                expiry_time_str = f" vào lúc {datetime.fromtimestamp(float(expiry_time_ts)).strftime('%H:%M:%S %d/%m/%Y')}"
            except (ValueError, TypeError, OSError):
...
async def muatt_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text_lines.extend(["\n🏦 <b>Thông tin thanh toán:</b>",
                       ...
                       f"   5️⃣ Bot sẽ tự động chuyển tiếp ảnh đến <b>@{context.bot.username}</b> (ID: {BILL_FORWARD_TARGET_ID}).",
    ...
    if not photo_sent:
        try:
            # <<< Sửa lỗi: Gửi text và nút bấm nếu ảnh lỗi >>>
...
# --- Lệnh /addtt (Đã sửa lỗi SyntaxError) ---
async def addtt_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_vip_data:
         try: # <<< Sửa lỗi SyntaxError ở đây
             ...
             logger.warning(f"Admin {admin_user.id}: Invalid expiry for user {target_user_id_str}. Activating new.")
...
async def run_treo_loop(user_id_str: str, target_username: str, context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    initial_info_sent = False # <<< Flag mới để chỉ gửi info lần đầu
    ...
    try:
        while True:
            ...
            if persistent_treo_configs.get(user_id_str, {}).get(target_username) != chat_id:
                 ...
                 if user_id_str in active_treo_tasks and target_username in active_treo_tasks[user_id_str]:
                      ...
                      # Chỉ xóa nếu task trong dict là task hiện tại
                      if current_task_in_dict is current_asyncio_task:
                          del active_treo_tasks[user_id_str][target_username]
                          if not active_treo_tasks[user_id_str]: del active_treo_tasks[user_id_str]
            ...
            api_data = api_result.get("data", {}) # Lấy data để hiển thị info
            gain = 0
            ...
            if success:
                ...
                # <<< Sửa lỗi: Gửi thông tin chi tiết hơn khi treo thành công lần đầu >>>
                if not initial_info_sent:
                    try:
                        f_before = api_data.get("followers_before", None) # Lấy giá trị gốc, có thể là số hoặc "?"
                        f_after = api_data.get("followers_after", None)
                        avatar = api_data.get("avatar", "")
                        tt_username = html.escape(api_data.get("username", target_username))
                        name = html.escape(str(api_data.get("name", "?")))

                        initial_lines = [f"🚀 Treo cho <a href='https://tiktok.com/@{tt_username}'>@{tt_username}</a> ({name}) bởi {invoking_user_mention} đã chạy thành công lần đầu!"]

                        # Hiển thị avatar nếu có
                        if avatar and avatar.startswith("http"):
                            # Gửi avatar riêng biệt
                            try:
                                await app.bot.send_photo(chat_id=chat_id, photo=avatar, caption="\n".join(initial_lines), parse_mode=ParseMode.HTML, disable_notification=True)
                                initial_lines = [] # Xóa caption nếu đã gửi cùng ảnh
                            except Exception as e_send_photo:
                                logger.warning(f"Failed to send avatar for initial treo info {task_name}: {e_send_photo}")
                                initial_lines.append(f"🖼️ (<a href='{html.escape(avatar)}'>Link ảnh đại diện</a>)") # Fallback link

                        # Hiển thị thông tin follow nếu có và hợp lệ
                        try:
                            # Cố gắng chuyển đổi sang số để so sánh và định dạng
                            f_before_num = int(re.sub(r'[^\d]', '', str(f_before))) if f_before is not None else None
                            f_after_num = int(re.sub(r'[^\d]', '', str(f_after))) if f_after is not None else None

                            if f_before_num is not None: initial_lines.append(f"📊 Follow trước khi chạy: <code>{f_before_num:,}</code>")
                            if gain > 0: initial_lines.append(f"✨ Lần tăng đầu tiên: <b>+{gain:,}</b>")
                            if f_after_num is not None: initial_lines.append(f"📈 Follow sau khi chạy: <code>{f_after_num:,}</code>")

                        except (ValueError, TypeError):
                            # Nếu không chuyển đổi được, hiển thị giá trị gốc nếu khác "?"
                            if f_before is not None and f_before != "?": initial_lines.append(f"📊 Follow trước khi chạy: <code>{html.escape(str(f_before))}</code>")
                            if gain > 0: initial_lines.append(f"✨ Lần tăng đầu tiên: <b>+{gain:,}</b>") # Gain đã là số
                            if f_after is not None and f_after != "?": initial_lines.append(f"📈 Follow sau khi chạy: <code>{html.escape(str(f_after))}</code>")

                        # Gửi tin nhắn text nếu chưa gửi cùng ảnh
                        if initial_lines:
        ...
        if user_id_str in active_treo_tasks and target_username in active_treo_tasks[user_id_str]:
            ...
            if current_task_in_dict is current_asyncio_task and current_asyncio_task and current_asyncio_task.done():
                del active_treo_tasks[user_id_str][target_username]
                if not active_treo_tasks[user_id_str]: del active_treo_tasks[user_id_str]
...
async def treo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if target_username:
        ...
        try:
            ...
            # Thông báo thành công (đơn giản hơn, chi tiết sẽ hiện sau)
...
async def dungtreo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args: # <<< Xử lý /dungtreo không có đối số (Dừng tất cả) >>>
        ...
        if stopped_count > 0:
             # <<< Sửa lỗi: Thông báo dừng tất cả thành công >>>
             await update.message.reply_html(f"✅ Đã dừng thành công <b>{stopped_count}</b> tài khoản đang treo.")
        else:
             await send_temporary_message(update, context, "ℹ️ Bạn hiện không có tài khoản nào đang treo để dừng.", duration=20, reply=False)
    else: # <<< Xử lý /dungtreo <username> (Dừng một target) >>>
        ...
        if stopped:
            ...
            # <<< Sửa lỗi: Thông báo dừng thành công cho target cụ thể >>>
            ...
            # <<< Sửa lỗi: Thông báo nếu không tìm thấy target cụ thể >>>
            await send_temporary_message(update, context, f"⚠️ Không tìm thấy cấu hình treo nào đang hoạt động hoặc đã lưu cho <code>@{html.escape(target_username_clean)}</code> để dừng.", duration=20, reply=False)

# --- Lệnh /listtreo (Đã sửa để lấy từ persistent) ---
async def listtreo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # <<< Sửa lỗi: Lấy danh sách từ persistent_treo_configs là nguồn chính xác nhất >>>
    ...
    if not treo_targets:
        ...
        # <<< Sửa lỗi: Chỉ lặp qua danh sách từ persistent_treo_configs >>>
        for target in sorted(treo_targets):
             # Kiểm tra trạng thái ước lượng từ active_treo_tasks (chỉ để hiển thị icon)
             is_running = False
...
async def mess_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # <<< Sửa lỗi: Gửi thông báo rõ ràng hơn >>>
...
async def report_treo_stats(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id_str, targets in stats_snapshot.items():
        if isinstance(targets, dict):
            ...
            logger.warning(f"[Stats Job] Invalid target structure for user {user_id_str} in snapshot.")
    ...
    # Chỉ thêm dòng này nếu không có ai trong top list
    if not user_mentions_cache:
...
async def shutdown_async_tasks(tasks_to_cancel: list[asyncio.Task], timeout: float = 2.0):
    ...
    for i, result in enumerate(results):
        ...
        if isinstance(result, asyncio.CancelledError):
            ...
            errors_count += 1
            logger.error(f"[Shutdown] Error occurred in task '{task_name}' during cancellation: {result}", exc_info=False)
        else:
            finished_count += 1
...
def main() -> None:
    ...
    application = (Application.builder().token(BOT_TOKEN).job_queue(JobQueue())
                   # Tăng timeout để xử lý API lâu hơn
                   .pool_timeout(120).connect_timeout(60).read_timeout(120).write_timeout(120)
    ...
    persistent_treo_snapshot = dict(persistent_treo_configs) # Lấy bản sao để lặp an toàn
    ...
    if persistent_treo_snapshot:
        for user_id_str, targets_for_user in persistent_treo_snapshot.items():
            try:
                ...
                # Lấy bản sao target của user để lặp an toàn
                ...
                for target_username, chat_id_int in targets_snapshot.items():
                    ...
                    if current_user_restored_count >= vip_limit:
                         ...
                         if user_id_str in persistent_treo_configs and target_username in persistent_treo_configs[user_id_str]:
                              del persistent_treo_configs[user_id_str][target_username]
                              # Sẽ save_data() sau khi dọn dẹp xong users_to_cleanup_restore
                         continue # Bỏ qua target này

                    # Kiểm tra xem task đã chạy chưa (trường hợp restart cực nhanh - hiếm)
                ...
                users_to_cleanup_restore.append(user_id_str) # Đánh dấu để dọn dẹp nếu có lỗi
    ...
    if users_to_cleanup_restore:
        ...
        for user_id_str_clean in unique_users_to_cleanup:
            if user_id_str_clean in persistent_treo_configs:
                ...
                cleaned_persistent_configs_on_restore = True # Đánh dấu đã có thay đổi
        if cleaned_persistent_configs_on_restore: logger.info(f"Removed persistent configs for {len(unique_users_to_cleanup)} users.")

    # Check lại xem có config nào bị xóa do vượt limit không (sau khi user hợp lệ đã được xử lý)
    for uid, targets_orig in persistent_treo_snapshot.items():
         # Chỉ check user còn tồn tại trong config (ko bị xóa do hết VIP/lỗi)
         if uid in persistent_treo_configs:
             if len(persistent_treo_configs.get(uid, {})) < len(targets_orig):
                 cleaned_persistent_configs_on_restore = True # Đánh dấu đã có thay đổi
                 logger.info(f"[Restore] Detected over-limit config removal for user {uid}.")
    ...
    if tasks_to_create_data:
        ...
        # Tạo context mặc định một lần để truyền vào task, tránh tạo nhiều lần
    ...
    try:
        ...
        if tasks_to_stop_on_shutdown:
            ...
            try:
                 ...
                 for task in tasks_to_stop_on_shutdown: task.cancel() # Fallback hủy trực tiếp
