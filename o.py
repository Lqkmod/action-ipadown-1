...
import telebot
import requests # Sử dụng requests cho các lời gọi API đồng bộ
import json
import html
import os
import time
...
LINK_SHORTENER_API_KEY = "cb879a865cf502e831232d53bdf03813caf549906e1d7556580a79b6d422a9f7" # API Key Yeumoney
# !!! ĐÃ SỬA: Thêm {key} vào cuối URL template !!!
BLOGSPOT_URL_TEMPLATE = "https://khangleefuun.blogspot.com/2025/04/key-ngay-body-font-family-arial-sans_11.html?m=1&ma={key}"
LINK_SHORTENER_API_BASE_URL = "https://yeumoney.com/QL_api.php" # API Yeumoney
# --- Rất khuyến khích dùng File ID thay vì URL ---
# !!! CẢNH BÁO: URL CDN này có thể hết hạn. Nên dùng File ID !!!
REPLY_VIDEO_URL = "https://v16m-default.tiktokcdn.com/2b5dc722c5cc43d098bc92c575b58fb0/67fcaefd/video/tos/useast2a/tos-useast2a-pve-0068/35184ccb253540699add4fba02d9fe88/?a=0&bti=OUBzOTg7QGo6OjZAL3AjLTAzYCMxNDNg&ch=0&cr=0&dr=0&er=0&lr=all&net=0&cd=0%7C0%7C0%7C0&cv=1&br=6658&bt=3329&cs=0&ds=6&ft=EeF4ntZWD03Q12NvR-pXeIxRSfYFpq_45SY&mime_type=video_mp4&qs=0&rc=OGVlZjloPDNkM2k6ZzVnN0BpM3E2Zjc6Zm47ZDMzNzgzM0A0LjNgLjEyXjExLWEuNV9eYSNqbW5icjQwbDNgLS1kLzZzcw%3D%3D&vvpl=1&l=2025041408444822579B7780883E3CEE16&btag=e000b8000"
# Thay bằng File ID: REPLY_VIDEO_URL = "<YOUR_VIDEO_FILE_ID>"
ADMIN_USER_ID = 7193749511   # <--- ID Admin của bạn
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - [%(levelname)s] - %(message)s', # Định dạng log đơn giản hơn
    handlers=[
...
try:
    ...
    logger.critical(f"Failed to create bot instance: {e}. Check BOT_TOKEN.", exc_info=True)
    exit(1) # Thoát nếu không tạo được bot

# --- 💾 Hàm Lưu/Tải Dữ liệu ---
def save_data():
    global valid_keys, activated_users, user_getkey_cooldown
    try:
        ...
        data_to_save = {
            ...
            "user_cooldowns": { "getkey": string_key_getkey_cooldown }
        }
        ...
        try:
            if ADMIN_USER_ID:
                bot.send_message(ADMIN_USER_ID, f"🚨 LỖI BOT: Không thể lưu dữ liệu!\nLỗi: {e}", disable_notification=True)
        except Exception: pass # Bỏ qua lỗi gửi thông báo
...
def load_data():
    ...
    try:
        if os.path.exists(DATA_FILE):
            ...
            logger.info(f"Data loaded from {DATA_FILE}. Keys: {len(valid_keys)}, Activated: {len(activated_users)}, Cooldowns: {len(user_getkey_cooldown)}")
        else:
            logger.info(f"{DATA_FILE} not found. Initializing empty data.")
            valid_keys, activated_users, user_getkey_cooldown = {}, {}, {}
    except json.JSONDecodeError as e:
        logger.error(f"Failed to parse {DATA_FILE}: {e}. Backing up and starting fresh.", exc_info=True)
        backup_filename = f"{DATA_FILE}.bak_{int(time.time())}"
        try:
            if os.path.exists(DATA_FILE): os.rename(DATA_FILE, backup_filename)
            logger.info(f"Backed up corrupted file to {backup_filename}")
        except OSError as backup_err: logger.error(f"Could not backup corrupted file: {backup_err}")
        valid_keys, activated_users, user_getkey_cooldown = {}, {}, {}
    except Exception as e:
        logger.error(f"Failed to load data: {e}. Using empty data.", exc_info=True)
        valid_keys, activated_users, user_getkey_cooldown = {}, {}, {}
...
def generate_random_key(length=8):
    ...
    while True:
        ...
        if key not in valid_keys: return key
...
def is_user_activated(user_id: int) -> bool:
    ...
    if expiry_time_str:
        try:
            ...
            if current_time < expiry_time: return True
            else:
                ...
                save_data() # Lưu khi xóa user hết hạn
                return False
        except (ValueError, TypeError):
            logger.warning(f"Invalid activation timestamp for user {user_id_str}. Removing.")
            if user_id_str in activated_users: del activated_users[user_id_str]
            save_data() # Lưu khi xóa user lỗi
            return False
    return False

# --- 🎬 Hàm Gửi Phản Hồi Chính ---
def send_response_with_video(message: telebot.types.Message, text: str, delete_original: bool = True, disable_web_page_preview: bool = True):
    if not message or not message.chat or not message.from_user: return None
    chat_id = message.chat.id
    user_id = message.from_user.id
    ...
    if REPLY_VIDEO_URL:
        try:
            bot.send_video(chat_id, REPLY_VIDEO_URL, timeout=30)
            video_sent = True
            logger.debug(f"Sent video to {user_id} in {chat_id}")
        except telebot.apihelper.ApiException as e: logger.error(f"Video send APIException: {e}")
        except Exception as e: logger.error(f"Video send unexpected error: {e}", exc_info=True)
    else: logger.warning("REPLY_VIDEO_URL not set.")

    # 2. Chuẩn bị Text
    random_emoji = random.choice(RANDOM_EMOJIS) + " " if RANDOM_EMOJIS else ""
    text_with_emoji = f"{random_emoji}{str(text)}" # Đảm bảo là string

    # 3. Cắt bớt nếu quá dài
    MAX_LEN = 4096
    TRUNCATE_SUFFIX = "...\n<i>(Nội dung quá dài)</i>"
    if len(text_with_emoji) > MAX_LEN:
        logger.warning(f"Msg > {MAX_LEN} chars, truncating.")
        max_original_len = MAX_LEN - len(random_emoji) - len(TRUNCATE_SUFFIX) - 5
        if max_original_len > 20:
            original_text = str(text)
            last_space = original_text[:max_original_len].rfind(' ')
            cutoff = last_space if last_space > max_original_len / 2 else max_original_len
            text_with_emoji = f"{random_emoji}{original_text[:cutoff]}{TRUNCATE_SUFFIX}"
        else: text_with_emoji = f"{random_emoji}⚠️ Nội dung quá dài."

    # 4. Gửi Text
    message_sent = False
    try:
        sent_text_msg = bot.send_message(chat_id, text_with_emoji, parse_mode='HTML', disable_web_page_preview=disable_web_page_preview)
        message_sent = True
        logger.info(f"Sent HTML msg to {user_id}")
    except telebot.apihelper.ApiException as e:
        logger.warning(f"HTML send failed: {e}")
        if any(err in str(e).lower() for err in ["parse", "entity", "tag"]):
            logger.info("Trying plain text fallback.")
            try:
                plain_text = f"{random_emoji}{re.sub('<[^<]+?>', '', str(text))}"
                if len(plain_text) > MAX_LEN: plain_text = plain_text[:MAX_LEN-4] + "..."
                sent_text_msg = bot.send_message(chat_id, plain_text, disable_web_page_preview=True)
                message_sent = True
                logger.info("Plain text fallback successful.")
            except Exception as fb_e: logger.error(f"Plain text fallback failed: {fb_e}", exc_info=True)
        else: logger.error(f"Unhandled API error sending message: {e}")
    except Exception as e: logger.error(f"Unexpected error sending text: {e}", exc_info=True)

    # 5. Xóa tin nhắn gốc
    if delete_original and (video_sent or message_sent):
        try:
            bot.delete_message(chat_id, original_message_id)
            logger.debug(f"Deleted original msg {original_message_id}")
        except Exception as e: logger.warning(f"Failed to delete original msg {original_message_id}: {e}")

    return sent_text_msg

# --- 🔑 Command Handlers ---

@bot.message_handler(commands=['bottiktok']) # Gộp start và help
def handle_start_help(message: telebot.types.Message):
    user_mention = f"<a href='tg://user?id={message.from_user.id}'>{html.escape(message.from_user.first_name)}</a>"
    # Lấy thông tin bot để hiển thị tên và link chính xác
    bot_info = bot.get_me()
    ...
    help_text = (
        f"👋 Chào mừng {user_mention} đến với <b>{bot_name}</b>!\n\n"
        f"<b>Các lệnh có sẵn:</b>\n"
        f"🔑 <code>/getkey</code> - Lấy link nhận Key kích hoạt <i>(Cooldown: {gk_cd_m} phút)</i>.\n"
        f"⚡️ <code>/nhapkey <key></code> - Nhập Key để kích hoạt.\n"
        f"📜 <code>/help</code> hoặc <code>/start</code> - Hiển thị trợ giúp này.\n\n"
        f"<b>Thông tin Key:</b>\n"
        f"⏳ Key chưa nhập sẽ hết hạn sau <b>{key_exp_h} giờ</b>.\n"
        f"✅ Sau khi nhập thành công, bạn sẽ được kích hoạt trong <b>{act_h} giờ</b>."
        # Thêm thông tin về các lệnh khác nếu tích hợp (/tim, /fl)
    )
    # Gửi tin nhắn chào mừng/trợ giúp (không cần video)
    random_emoji = random.choice(RANDOM_EMOJIS) + " " if RANDOM_EMOJIS else ""
    try:
        # Gửi dưới dạng tin nhắn mới thay vì reply để tránh lỗi khi lệnh gốc bị xóa
        bot.send_message(message.chat.id, f"{random_emoji}{help_text}", disable_web_page_preview=True)
        # Chỉ xóa lệnh gốc nếu không phải chat riêng (tùy chọn)
        if message.chat.type != "private":
             try: bot.delete_message(message.chat.id, message.message_id)
             except: pass
    except Exception as e:
         logger.error(f"Error sending start/help message: {e}")

@bot.message_handler(commands=['getkey'])
def handle_getkey(message: telebot.types.Message):
    user_id = message.from_user.id
    ...
    # 1. Check Cooldown
    last_usage = user_getkey_cooldown.get(user_id_str, 0)
    if (current_time - last_usage) < GETKEY_COOLDOWN_SECONDS:
        remaining = int(GETKEY_COOLDOWN_SECONDS - (current_time - last_usage))
        cooldown_msg = f"⏳ {user_mention}, vui lòng đợi <b>{remaining} giây</b> nữa để dùng lại <code>/getkey</code>."
        send_response_with_video(message, cooldown_msg, delete_original=True)
        return

    # 2. Tạo Key & URL Đích
    generated_key = generate_random_key()
    # Sử dụng f-string để nhúng key vào template
    ...
    logging.info(f"User {user_id} requesting key. Key: {generated_key}.")

    final_response_text = ""
    key_saved_to_dict = False
    bot_info = bot.get_me() # Lấy thông tin bot

    try:
        # 3. Lưu key tạm thời (trước khi gọi API)
        ...
        save_data() # Lưu ngay
        logger.info(f"Key {generated_key} saved for user {user_id}.")

        # 4. Gọi API Rút gọn Link
        try:
             headers = {'User-Agent': f'TelegramBot/{bot_info.username}'}
             response = requests.get(LINK_SHORTENER_API_BASE_URL, params=shortener_params, headers=headers, timeout=30)
             response.raise_for_status()
             response_data = response.json()

             # 5. Xử lý Phản hồi API
             if response.status_code == 200 and response_data.get("status") == "success":
                generated_short_url = response_data.get("shortenedUrl")
                if generated_short_url and isinstance(generated_short_url, str) and generated_short_url.startswith("http"):
                    user_getkey_cooldown[user_id_str] = time.time(); save_data() # Cập nhật cooldown và lưu
                    key_exp_h = KEY_EXPIRY_SECONDS // 3600
                    final_response_text = (
                        ...
                        f"│   2️⃣ Làm theo các bước trên trang web.\n"
                        f"│   3️⃣ Nhận mã Key dạng <code>Dinotool-xxxx</code>.\n"
                        f"│   4️⃣ Quay lại đây và dùng lệnh:\n"
                        f"│      <code>/nhapkey <key_cua_ban></code>\n\n"
                        f"│ ⏳ <i>Key cần được nhập trong vòng <b>{key_exp_h} giờ</b>.</i>\n\n"
                        f"╰─── ⋅ ⋅ ─── 🤖 <a href='https://t.me/{bot_info.username}'>{html.escape(bot_info.first_name)}</a> ⋅ ⋅ ───╯"
                    )
                    key_saved_to_dict = False # Đánh dấu không cần xóa key nữa
                else:
                    logger.error(f"API success but invalid URL: {generated_short_url}")
                    final_response_text = "❌ <b>Lỗi Tạo Link:</b> API trả về link không hợp lệ."
             else:
                 api_message = response_data.get("message", f"Status: {response_data.get('status', 'N/A')}")
                 logger.error(f"API error. User: {user_id}. Status: {response.status_code}. Resp: {response_data}")
                 final_response_text = f"❌ <b>Lỗi Tạo Link:</b> <code>{html.escape(str(api_message))}</code>."

        # Xử lý lỗi request
        except requests.exceptions.Timeout: final_response_text = "❌ <b>Lỗi Timeout</b> khi tạo link."
        except requests.exceptions.ConnectionError: final_response_text = "❌ <b>Lỗi Kết Nối</b> đến API tạo link."
        except requests.exceptions.HTTPError as e: final_response_text = f"❌ <b>Lỗi API</b> (Mã: {e.response.status_code}). Liên hệ admin."
        except requests.exceptions.RequestException as e: final_response_text = f"❌ <b>Lỗi Mạng:</b> {e}"
        except json.JSONDecodeError: final_response_text = "❌ <b>Lỗi API:</b> Phản hồi không đúng định dạng."
        except Exception as e:
             logger.error(f"Unexpected error in /getkey API call: {e}", exc_info=True)
             final_response_text = "❌ <b>Lỗi Hệ Thống</b> không mong muốn."

    finally:
        if key_saved_to_dict and generated_key in valid_keys:
             logger.warning(f"Removing key {generated_key} due to error.")
             try: del valid_keys[generated_key]; save_data()
             except Exception as e: logger.error(f"Error saving after key removal: {e}")
        send_response_with_video(message, final_response_text, delete_original=True, disable_web_page_preview=False)

@bot.message_handler(commands=['nhapkey'])
def handle_nhapkey(message: telebot.types.Message):
    user_id = message.from_user.id
    user_mention = f"<a href='tg://user?id={user_id}'>{html.escape(message.from_user.first_name)}</a>"
    current_time = time.time()
    user_id_str = str(user_id)
    ...
    if len(args) < 2 or not args[1].strip(): err_txt = "⚠️ Thiếu key. Ví dụ: <code>/nhapkey Dinotool-ABCXYZ</code>"
    else:
        key = args[1].strip()
        if not re.match(r"^Dinotool-[A-Za-z0-9]{4,}$", key): err_txt = f"⚠️ Key <code>{html.escape(key)}</code> không đúng định dạng."
        else: submitted_key = key

    if err_txt:
        send_response_with_video(message, err_txt, delete_original=True)
        return

    # 2. Xác thực Key
    logger.info(f"User {user_id} activating with key: '{submitted_key}'")
    ...
    elif "expiry_time" not in key_data:
        logger.error(f"Corrupted key data (no expiry): {submitted_key}")
        final_response_text = "❌ <b>Lỗi dữ liệu key.</b> Liên hệ admin."
        if submitted_key in valid_keys: del valid_keys[submitted_key]; save_data()
    elif current_time > float(key_data.get("expiry_time", 0)):
        expiry_str = time.strftime('%H:%M %d/%m/%Y', time.localtime(float(key_data["expiry_time"])))
        final_response_text = f"❌ Key <code>{html.escape(submitted_key)}</code> đã hết hạn nhập ({expiry_str}). Lấy key mới bằng <code>/getkey</code>."
        if submitted_key in valid_keys: logger.info(f"Removing expired key {submitted_key}."); del valid_keys[submitted_key]; save_data()
    elif key_data.get("used_by") is not None:
        used_by_id = str(key_data["used_by"])
        if used_by_id == user_id_str:
            if is_user_activated(user_id):
                 expiry_val = float(activated_users.get(user_id_str, 0))
                 final_response_text = f"✨ Bạn đã được kích hoạt đến <b>{time.strftime('%H:%M %d/%m/%Y', time.localtime(expiry_val))}</b>."
            else: # User dùng key này rồi nhưng hết hạn kích hoạt cũ -> Kích hoạt lại
                 activation_expiry = current_time + ACTIVATION_DURATION_SECONDS
                 activated_users[user_id_str] = activation_expiry; save_data()
                 act_h = ACTIVATION_DURATION_SECONDS // 3600
                 final_response_text = f"✅ <b>Kích hoạt lại thành công!</b> (Hiệu lực {act_h} giờ)"
        else: final_response_text = f"❌ Key <code>{html.escape(submitted_key)}</code> đã bị người khác sử dụng."
    else: # Key hợp lệ, chưa dùng, còn hạn -> Kích hoạt
        try:
            key_data["used_by"] = user_id
            activation_expiry = current_time + ACTIVATION_DURATION_SECONDS
            activated_users[user_id_str] = activation_expiry
            save_data()
            logger.info(f"User {user_id} activated with key {submitted_key}. Expires: {time.strftime('%Y-%m-%d %H:%M', time.localtime(activation_expiry))}")
            act_h = ACTIVATION_DURATION_SECONDS // 3600
            expiry_str_fmt = time.strftime('%H:%M %d/%m/%Y', time.localtime(activation_expiry))
            final_response_text = (
                f"╭─── ⋅ ⋅ ─── ✅ <b>KÍCH HOẠT THÀNH CÔNG</b> ✅ ─── ⋅ ⋅ ───╮\n\n"
                f"│ Chúc mừng {user_mention}!\n\n"
                f"│ 🔑 Key đã sử dụng: <code>{html.escape(submitted_key)}</code>\n"
                f"│ ✨ Bạn có thể sử dụng các lệnh khác.\n" # Điều chỉnh nếu có lệnh khác
                f"│ ⏳ Hiệu lực đến: <b>{expiry_str_fmt}</b> <i>(~{act_h} giờ)</i>\n\n"
                f"╰─── ⋅ ⋅ ─── 🤖 <a href='https://t.me/{bot_info.username}'>{html.escape(bot_info.first_name)}</a> ⋅ ⋅ ───╯"
            )
        except Exception as e:
             logger.error(f"Activation process error: {e}", exc_info=True)
             final_response_text = "❌ <b>Lỗi hệ thống</b> khi kích hoạt."

    # 3. Gửi Phản hồi
    send_response_with_video(message, final_response_text, delete_original=True)
...
if __name__ == '__main__':
    ...
    try:
        me = bot.get_me()
        logger.info(f"Bot initialized: ID={me.id}, Name='{me.first_name}', User=@{me.username}")
    except Exception as e:
        logger.critical(f"Failed to get bot info: {e}. Check BOT_TOKEN.", exc_info=True)
        exit(1)

    logger.info("Starting polling...")
    try:
        bot.polling(none_stop=True, interval=1, timeout=30) # Thêm timeout cho polling
    except Exception as e:
        logger.critical(f"CRITICAL POLLING ERROR: {e}", exc_info=True)
    finally:
        logger.info("Shutdown sequence initiated. Saving data...")
        save_data()
        logger.info("Data save attempt complete. Bot exiting.")
