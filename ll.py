...
# QR_CODE_URL = "https://i.imgur.com/49iY7Ft.jpeg" # Link ảnh QR Code - KHÔNG CÒN DÙNG NỮA
...
async def lenh_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Sửa mô tả /fl cho phép gạch dưới
    cmd_lines.append(f"  <code>/fl &lt;username&gt;</code> - Tăng follow cho tài khoản TikTok (username có thể chứa '_', ⏳ {tf_cd_m}p/user)")
    cmd_lines.append("\n<b><u>👑 Lệnh VIP:</u></b>")
    cmd_lines.append(f"  <code>/muatt</code> - Thông tin và hướng dẫn mua VIP")
    # Sửa mô tả /treo cho phép gạch dưới
    cmd_lines.append(f"  <code>/treo &lt;username&gt;</code> - Tự động chạy <code>/fl</code> mỗi {treo_interval_m} phút (username có thể chứa '_', Dùng slot)")
...
async def fl_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    err_txt = None
    # SỬA ĐỔI REGEX Ở ĐÂY: Thêm dấu gạch dưới (_) vào bộ ký tự cho phép
    username_regex = r"^[a-zA-Z0-9._]{2,24}$" # <-- SỬA ĐỔI Ở ĐÂY
    ...
    if not args:
        ...
        if not uname: err_txt = "⚠️ Username không được trống."
             ...
             # SỬA ĐỔI THÔNG BÁO LỖI Ở ĐÂY: Thêm _ vào danh sách ký tự hợp lệ
            err_txt = (f"⚠️ Username <code>{html.escape(uname_raw)}</code> không hợp lệ.\n"
                       f"(Chỉ chứa chữ, số, '.', '_', dài 2-24 ký tự)") # <-- SỬA ĐỔI Ở ĐÂY
...
# --- Lệnh /muatt (SỬA: Không gửi QR) ---
async def muatt_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Hiển thị thông tin mua VIP và nút yêu cầu gửi bill (chỉ text)."""
    if not update or not update.message: return
    chat_id = update.effective_chat.id
    user = update.effective_user
    if not user: return
    ...
    # Chỉ gửi tin nhắn văn bản kèm nút bấm
    try:
        await context.bot.send_message(chat_id=chat_id, text=text, parse_mode=ParseMode.HTML,
                                       disable_web_page_preview=True, reply_markup=keyboard)
        logger.info(f"Sent /muatt text info with prompt button to user {user_id} in chat {chat_id}")
    except (BadRequest, Forbidden, TelegramError) as e:
        logger.error(f"Error sending /muatt text message to chat {chat_id}: {e}")
    except Exception as e_unexp:
        logger.error(f"Unexpected error sending /muatt command text to chat {chat_id}: {e_unexp}", exc_info=True)
    ...
    try:
        while True:
            ...
            try:
                ...
                if success:
                    status_lines.append(f"✅ : Treo <code>@{html.escape(target_username)}</code> thành công!")
...
# --- Lệnh /treo (VIP - Cập nhật để lưu persistent config, cho phép '_') ---
async def treo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args: err_txt = ("⚠️ Chưa nhập username TikTok cần treo.\n<b>Cú pháp:</b> <code>/treo username</code>")
        ...
        if not uname: err_txt = "⚠️ Username không được trống."
             ...
             # SỬA ĐỔI THÔNG BÁO LỖI Ở ĐÂY: Thêm _ vào danh sách ký tự hợp lệ
             err_txt = (f"⚠️ Username <code>{html.escape(uname_raw)}</code> không hợp lệ.\n(Chữ, số, '.', '_', dài 2-24)") # <-- SỬA ĐỔI Ở ĐÂY
...
def main() -> None:
    ...
    # Đặt group=-1 để chạy trước các handler command/message mặc định (group=0)
