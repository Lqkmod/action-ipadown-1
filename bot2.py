...
async def make_api_request(url: str, params: dict | None = None, method: str = "GET", timeout: float = API_TIMEOUT_SECONDS) -> dict:
    ...
    try:
        ...
        # <<< THÊM HEAD METHOD nếu cần kiểm tra kích thước file >>>
...
async def run_treo_loop(user_id_str: str, target_username: str, context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    try:
        while True:
            ...
            # VÀ CHỈ GỬI KHI KHÔNG PHẢI LÀ LẦN THÀNH CÔNG BAN ĐẦU (tránh gửi 2 tin nhắn liên tục)
            # is_first_success = success and not initial_info_sent # Đây có phải lần thành công đầu tiên không?
            # should_send_status = (initial_info_sent or not success or (success and gain > 0)) and not is_first_success
...
# <<< SỬA LỖI: Đảm bảo khối try...except...finally này được thụt lề đúng (ngang cấp với if) >>>
if __name__ == "__main__":
    try:
        ...
        # Khối try...except bên trong này phải thụt vào so với except bên ngoài
        try:
    ...
    # finally phải cùng cấp thụt lề với try và except bao ngoài cùng
    finally:
        ...
        if http_client:
            ...
            # Khối try...except bên trong này phải thụt vào so với finally
            try:
                loop = asyncio.get_event_loop_policy().get_event_loop()
                if loop.is_running(): loop.run_until_complete(http_client.aclose())
