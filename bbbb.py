...
import re # Đảm bảo đã import re ở đầu file của bạn
import html # Đảm bảo đã import html
# ... (các import khác)
...
async def tim_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- Kiểm tra quyền và cooldown (Giữ nguyên) ---
        ...
        if api_response["success"] and isinstance(api_response["data"], dict):
            ...
            if is_api_success:
                ...
                if di_raw != "?": # Kiểm tra nếu giá trị không phải mặc định "?"
                    ...
                    # except phải cùng cấp thụt lề với try
                    except (ValueError, TypeError):
